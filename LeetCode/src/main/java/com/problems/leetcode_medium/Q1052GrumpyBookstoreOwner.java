...
public class Q1052GrumpyBookstoreOwner {
    ...
    private static class Solution {
        public int maxSatisfied(int[] customers, int[] grumpy, int minutes) {
            ...
            while (r < n) {
                // when owner is not grumpy, accumulate satisfaction
                sum += customers[r] * (1 - grumpy[r]);
                // expand sliding window right edge, accumulate satisfaction when owner is grumpy
                extra += customers[r] * grumpy[r];
                // capture the maximum value of the sliding window
